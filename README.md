...
- **Skills corpus** — 15 skills and 160 reference topics covering the complete HIG (foundations, components, patterns, inputs, platforms, technologies). Snapshot re-verified against the live HIG on 2026-09-10; includes the WWDC 2025 "Liquid Glass" overhaul, the WWDC 2026 / iOS 27 day-one updates (Siri AI, Snippets, reintroduced Design principles, Liquid Glass refinements), and the September 2026 iPhone Duo guidance. Canonical content remains at [developer.apple.com/design/human-interface-guidelines](https://developer.apple.com/design/human-interface-guidelines/).
...
Skills content is a snapshot taken 2026-06-11, re-verified and enriched 2026-07-08 (converter gained `tabNavigator`/`small`/`links` rendering), re-verified 2026-08-24 (Apple-side link normalization plus a handful of minor prose edits; topic tree unchanged at 172 topics), and refreshed 2026-09-10 (new Designing for iPhone Duo page, Layout and SharePlay rewrites; 173 topics). Refreshes are driven by the `scripts/hig-ingest/` pipeline, which walks Apple's static DocC render JSON (`developer.apple.com/tutorials/data/design/human-interface-guidelines/<slug>.json`) — the HIG pages are JS-rendered, but the underlying JSON is static and complete, so the scan is automatable end to end:
