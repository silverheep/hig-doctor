...
Skills content is a snapshot taken 2026-06-11, re-verified and enriched 2026-07-08 (converter gained `tabNavigator`/`small`/`links` rendering), and re-verified 2026-08-24 (Apple-side link normalization plus a handful of minor prose edits; topic tree unchanged at 172 topics). Refreshes are driven by the `scripts/hig-ingest/` pipeline, which walks Apple's static DocC render JSON (`developer.apple.com/tutorials/data/design/human-interface-guidelines/<slug>.json`) — the HIG pages are JS-rendered, but the underlying JSON is static and complete, so the scan is automatable end to end:
